  ...
  return async (req, res, next) => {
    try {
      ...
      if (roles.includes('Admin')) {
        ...
        if (!existingUser) {
          return res.status(401).send({ code: 401, message: 'Invalid login attempt for user (1)' });
        }
      }
      ...
      if (roles && roles.length > 0) {
        ...
        if (!userHasRequiredRole) return res.status(401).send({ code: 401, message: 'Unauthorized access' });
      }
